...
RUN mkdir -p /tevis/src/css
RUN mkdir -p /tevis/src/js
...
COPY templates/*.ico templates/

COPY src/js/*.js src/js/
...
#HEALTHCHECK --timeout=3s CMD curl -f http://localhost/api/ping || exit 1
